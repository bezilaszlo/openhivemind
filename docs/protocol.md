...
| Injected records | The `local-command-caveat` ahead of a `/command` is a whole `user` record flagged `isMeta: true` (measured 47/47 on real transcripts) and is dropped entire, never emitted. The `/command` echo itself is not flagged isMeta (measured 1/51) and its `command-name`/`command-message`/`command-args` tags appear in either order with `command-args` sometimes absent, so each tag is stripped independently rather than isMeta-dropped. A `system-reminder` is different again: it is stapled onto a record that still carries the developer's own typed text (memory recalls, hook output), so it is stripped wherever it occurs in the block — leading, mid-block, or trailing — rather than the record being dropped. Any record reduced to nothing once its wrappers are stripped is dropped |
