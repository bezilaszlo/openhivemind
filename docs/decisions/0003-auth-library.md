...
3. **Set `accountLinking.enabled: false`** (email auto-link is on by default)
   ...
   subject.
4. **Invites without SMTP.** `sendInvitationEmail` is configured as a no-op
   that returns the link; the admin UI shows it as copyable. SMTP later.
5. **Two roles.** The plugin's `owner` is the creating admin and is never
   exposed as assignable; access control defined as admin|member.
...
9. **Schema lives in our Drizzle tree**, generated then owned; table names may
   be remapped; auth tables in their own Postgres schema.
